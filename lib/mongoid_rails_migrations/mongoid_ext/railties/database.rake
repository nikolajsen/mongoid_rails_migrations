namespace :db do
  namespace :mongo do
    ...
    desc "Migrate the database through scripts in db/mongo/migrate or the paths you sepcify with PATH=a/b/c,a/b/d,a/b/e. Target specific version with VERSION=x. Turn off output with VERBOSE=false."
    task :migrate => :environment do
      Mongoid::Migration.verbose = ENV["VERBOSE"] ? ENV["VERBOSE"] == "true" : true
      path = ENV['PATH'] ? ENV['PATH'].split(':') : Mongoid::Migrator.migrations_path
    ...
    namespace :migrate do
      ...
      desc 'Runs the "up" for a given migration VERSION. Overwrite the default script path with PATH=a/b/c,a/b/d,a/b/e.'
      ...
      desc 'Runs the "down" for a given migration VERSION. Overwrite the default script path with PATH=a/b/c,a/b/d,a/b/e.'
    ...
    desc 'Rolls the database back to the previous migration. Specify the number of steps with STEP=n. Overwrite the default script path with PATH=a/b/c,a/b/d,a/b/e.'
